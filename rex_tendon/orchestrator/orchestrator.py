...
class OrchestratorApp:
    ...
    def __init__(
        ...
        # Hand pose tracking for wrist flex control
        self.hand_pose_tracker: Optional[HandPoseTracker] = None
        self.last_flexion_action_sent: float = 0.0
        self.flexion_action_interval: float = 0.1
    ...
    async def start(self) -> None:
        ...
        logger.info("Starting Rex Tendon...")
        # Store running loop reference
        self.loop = asyncio.get_running_loop()

        # Initialize hand pose tracker
        self.hand_pose_tracker = HandPoseTracker(smoothing_frames=5)
                ...
                if xy_l_finger is not None and xy_r_finger is not None:
                    ...
                    try:
                        ...
                        if point_3d is not None:
                            ...
                            # Wrist/hand tracking for direct robot control
                            # - Hand direction controls robot rotation (left/right)
                            # - Wrist Y position controls robot up/down
                            if (
                                self.hand_pose_tracker is not None
                                and mp_results_left is not None
                                and mp_results_left.multi_hand_landmarks
                            ):
                                hand_landmarks = mp_results_left.multi_hand_landmarks[0]
                                direction_angle, wrist_y = get_hand_pose_for_robot_control(
                                    hand_landmarks, left_frame.shape
                                )

                                smoothed_angle, smoothed_wrist_y, angle_delta, wrist_y_delta = (
                                    self.hand_pose_tracker.update(direction_angle, wrist_y)
                                )

                                if angle_delta is not None and wrist_y_delta is not None:
                                    from ..control.geometry import cursor_to_motor_positions

                                    angle_threshold = 0.3
                                    wrist_threshold = 0.1

                                    if (abs(angle_delta) > angle_threshold or abs(wrist_y_delta) > wrist_threshold):
                                        if current_time - self.last_flexion_action_sent > self.flexion_action_interval:
                                            self.last_flexion_action_sent = current_time

                                            cursor_x = np.clip(angle_delta * 2.0, -1.0, 1.0)
                                            cursor_y = np.clip(-wrist_y_delta * 2.0, -1.0, 1.0)

                                            if self.motor_controller and self.motor_controller.is_connected:
                                                try:
                                                    calibrated_ticks = self.motor_controller.get_calibration_data()
                                                    target_positions, _ = cursor_to_motor_positions(
                                                        np.array([cursor_x, cursor_y], dtype=np.float32),
                                                        calibrated_ticks,
                                                        noise_scale=0.0,
                                                    )
                                                    self.motor_controller.set_positions(target_positions)
                                                    logger.info(
                                                        "[HAND CONTROL] cursor=(%.2f, %.2f) angle_delta=%.2f wrist_delta=%.2f",
                                                        cursor_x, cursor_y, angle_delta, wrist_y_delta,
                                                    )
                                                except Exception as e:
