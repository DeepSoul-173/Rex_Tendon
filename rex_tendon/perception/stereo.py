...
def triangulate_points(
    ...
    *,
    ...
    point_3d[0] = np.clip(
        point_3d[0],
        coordinate_limits["X"]["clip_min"] if coordinate_limits else -np.inf,
        coordinate_limits["X"]["clip_max"] if coordinate_limits else np.inf,
    )
    point_3d[1] = np.clip(
        point_3d[1],
        coordinate_limits["Y"]["clip_min"] if coordinate_limits else -np.inf,
        coordinate_limits["Y"]["clip_max"] if coordinate_limits else np.inf,
    )
    point_3d[2] = np.clip(
        point_3d[2],
        coordinate_limits["Z"]["clip_min"] if coordinate_limits else -np.inf,
        coordinate_limits["Z"]["clip_max"] if coordinate_limits else np.inf,
    )
